...
animateFlag=false;}}})(i));});}
function changeImg(dur,imgWidth,page,elems){elems=(page=="index")?$(".banner a"):$(".imageFrame div div");switch(dur){case"after":$(elems[0]).parent().append(elems[0]).end().css({"left":(elems.length-1)*imgWidth*100+"%"});break;case"before":$(elems[elems.length-1]).parent().prepend(elems[elems.length-1]).end().css({"left":"-"+imgWidth*100+"%"});break;}}
...
$(elems).each(function(i,e){$(e).css({"left":i*imgWidth*100+"%"});});changeTab(elems,dotElems,page);}
